...
"""
Zeta Zeros Analysis Script

This script analyzes zeta zeros extracted from LMFDB format and explores their
relationship to the Unified Fractal Resonance Framework (UFRF).

Usage:
  python3 analyze_extracted_zeta.py [input_file] [output_dir]

Example:
  python3 analyze_extracted_zeta.py zeta_zeros.txt results/
"""
...
# Default values
DEFAULT_INPUT_FILE = "zeta_zeros.txt"
DEFAULT_OUTPUT_DIR = "results"

def load_zeta_zeros(file_path):
    """
    Load zeta zeros from text file
    ...
    try:
        ...
        print(f"Error loading zeta zeros: {str(e)}")
            ...
            for line_num, line in enumerate(f, 1):
                try:
                    ...
                    if line_num % 100000 == 0:
                        print(f"Loaded {line_num} zeros...")
                except ValueError:
                    print(f"Warning: Could not convert line {line_num} to float: '{line.strip()}'")
...
def analyze_metacycle_structure(zeros, metacycle_size=13):
    ...
    for lag in range(1, max_lag + 1):
        ...
        # Fix: Use the same number of bins for both dimensions
        n_bins = 20
        ...
        # Fix: Ensure px and py have the same shape as the flattened pxy
        ...
        # Create the outer product and ensure it has the same shape as pxy_flat
        outer_product = np.outer(px, py).flatten()
        outer_product = outer_product[:len(pxy_flat)]
        
        # Calculate mutual information
        mi = np.sum(pxy_flat * np.log(pxy_flat / outer_product))
...
def plot_entropy_analysis(entropy_values, anomalies, output_file):
    ...
    # Plot threshold
    plt.axhline(y=3.5, color='g', linestyle='--', label='Threshold (3.5)')
...
def analyze_zeta_zeros(input_file, output_dir):
    """
    Analyze zeta zeros and generate reports and visualizations
    
    Args:
        input_file: Path to the file containing zeta zeros
...
def main():
    ...
    print(f"Analyzing zeta zeros from {input_file}, saving results to {output_dir}")
    success = analyze_zeta_zeros(input_file, output_dir)
